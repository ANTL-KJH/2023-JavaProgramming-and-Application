...
public class BalancedBinarySearchTreeNode<E> {
    ...
    // value compare func, return this.value - other.value
    public int compareKey(E otherEntry) {
        int cmpResult = 0;
        if (this._entry instanceof Integer)// Integer
        {
            cmpResult = (int) (this._entry) - (int) otherEntry;
        } else if (this._entry instanceof Double)// Double
        {
            if ((double) (this._entry) > (double)otherEntry)
                ...
                cmpResult = 0;
        } else if (this._entry instanceof String)// String
        {
