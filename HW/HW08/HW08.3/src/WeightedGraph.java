...
class WeightedGraph {
    ...
    public Vertex addVertex(String vrtxName) {
        if (this.findVertex(vrtxName) == null) {
            ...
            this.vrtxArray.add(newVrtx);    // add vertex array
            newVrtx.setVID(this.num_vrtx);
            adjList.add(newVrtx.getVID(), new ArrayList<Vertex>()); // add adjlist
            this.num_vrtx++;
            return newVrtx;
        } else {
            return null;
    ...
    public Vertex findVertex(String vrtxName) {
        ...
        for (Vertex v : this.vrtxArray) {
            vname = v.getName();
            if (vname.equals(vrtxName)) // vertex name check
                return v;
        }
        return null;
    }
...
//creating a new node (from source to destination) in the adjacency list
    ...
    public void printDistTable() // printout distance table
    {
        int edge_weight;
        System.out.print("Distance Table :\n");
        System.out.print("     |");
        for(int i=0; i<this.num_vrtx; i++) {
    ...
    public List<Vertex> BreadthFirstSearch(Vertex start, Vertex dest) {
        ...
        start.level = 0; start.visited = true; start.prev = start;
    ...
    public List<Vertex> DijkstraSortestPath(Vertex start, Vertex dest) {
        ...
        for(Vertex v : vrtxArray) {
            if (v.equals(start)) {
                ...
                remainVrtxs.add(v);
                v.visited = false;
                v.accDist = this.distTbl[vid_start][v.vid];
                v.prev = start;
            }
        }
    ...
    public void printWeightedGraph(){
        System.out.printf("Vertices (num_vrtx = %d) :".formatted(this.num_vrtx));
        for (Vertex vrtx:this.vrtxArray)
            System.out.printf(" %s".formatted(vrtx));
        System.out.println("\nAdjacency List for the Graph is:");
        for (Vertex vrtx:this.vrtxArray)
        {
            System.out.printf("%s -> [ ".formatted(vrtx));
            for (Vertex v:this.adjList.get(vrtx.getVID()))
                System.out.printf("%s, ",v);    // print out adj vertex
            System.out.println("]");
        }
    }
}
